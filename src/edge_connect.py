...
from src.utils import Progbar, create_dir, stitch_images, imsave, getVGGModel
...
class EdgeConnect():
    ...
    def __init__(self, config):
        ...
        self.refine_model = RefineModel(config).to(config.DEVICE)
        ...
        while keep_training:
            ...
            for items in train_loader:
                ...
                # joint model
                    ...
                    r_outputs, r_gen_loss, r_dis_loss, r_logs = self.refine_model.process(images, masks, outputs_merged, vgg_model, selected_layers)
                    r_output_merged = (r_outputs * masks) + (images * (1 - masks))

                    # metrics
                    psnr = self.psnr(self.postprocess(images), self.postprocess(r_outputs))
                    mae = (torch.sum(torch.abs(images - r_outputs)) / torch.sum(images)).float()
                    ...
                    # backward
                    self.refine_model.backward(r_gen_loss, r_dis_loss)
